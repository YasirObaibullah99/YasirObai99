...
import heroImg from "../images/icon/C.png";  
...
function Skill4({directionLeft} : Props){
  return (
    <div className="group relative flex cursor-pointer">
        ...
        ease-in-out">     
        <Image 
        className="flex items-center justify-center rounded-full border border-gray-500 object-cover 
        w-20 h-20 xl:w-20 xl:h-20 filter group-hover:grayscale transition duration-300 
        ease-in-out" 
        src={heroImg} alt=""/>
        </motion.div>        
        ...
        ease-in-out group-hover:bg-white h-20 w-20 md:h-20 md:w-20 xl:w-20 xl:h-20 rounded-full z-0">
          <div className="flex items-center justify-center h-full">
            <p className="flex-3xl font-bold text-black opacity-100">86%</p>
          </div>
        </div>
    </div>
  )
